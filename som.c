...
void* getsom(dataVector* data, somConfig *config, dataBoundary* boundaries, short silent)
{
    ...
    while(again)
    {
        ...
        cfg.sigma = max(1.0E-10, cfg.sigma*cfg.sigmaDecreaseRate);
        short flagUpdated = 0;
        ...
        if(cfg.useNeighboursMethod && episode == cfg.useNeighboursTrigger)
        {
...
void writeNeuron(FILE* fp, somNeuron* n, somScore* score, long stepId, int p)
{
    ...
    fprintf(fp, "];%d;%d;%d;%ld;%d;%d;%d;%d;%d;%d", x,y,z , stepId, s, class,  class2, stabilized, classtotalEntries, class2totalEntries);
    fputs(";[", fp);
    int limit = s-1;
    for(int i=0;i<s;i++)
    {
